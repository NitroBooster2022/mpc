...
class MPC:
    def __init__(self, gazebo, useIMU=False, useEkf=False, sign=False):
        ...
        self.v_ref = -0.753*1/2
        self.steer_ref = 0.0
        self.xy_cost = 2#*2
        self.yaw_cost = 0.5#.5
        self.v_cost = 0 
        self.steer_cost = 0.0#.25
        self.delta_v_cost = 0.1
        self.delta_steer_cost = 0.
        ...
        wpts = np.array([[2.37, 13.3],[2.5, 13.32],[2.8, 13.36], [3.214,13.644]]).T - np.array([2.37, 13.3]).reshape(2,1)
        runs.append(wpts)
        ...
        self.x_max = np.amax(self.waypoints_x)+1
        self.x_min = np.amin(self.waypoints_x)-1
        self.y_max = np.amax(self.waypoints_y)+1
        self.y_min = np.amin(self.waypoints_y)-1
        self.waypoints_x = self.x_max - self.waypoints_x
        self.x_max = np.amax(self.waypoints_x)+1
        self.x_min = np.amin(self.waypoints_x)-1
        print("wpt x: ", self.waypoints_x)
        start_index = int(len(self.waypoints_x)*0)
        end_index = int(len(self.waypoints_x)*0.9)
        ...
        self.init_state = np.array([self.waypoints_x[start_index], self.waypoints_y[start_index], theta_ref+np.pi])
        ...
        print("input refs: ", np.around(self.input_refs, 3))
        exit()
...
if __name__ == '__main__':
    ...
    ## draw function
    draw_result = Draw_MPC_tracking(u=mpc.u_c, obstacle = mpc.obstacle, rob_diam=0.3, init_state=mpc.init_state, 
