...
class MPC:
    def __init__(self, gazebo, useIMU=False, useEkf=False, sign=False):
        ...
        wpts = np.array([[3.0745, -11.6686],[3.065077, -11.1888],[2.985, -10.7428], [2.289146,-10.461288]]).T
        #subtract by [3.0745, -11.6686]
        print("shape1: ",wpts.shape)
        wpts[0] -= 3.0745
        wpts[1] -= -11.6686
        runs= [wpts]
        ...
        for i in range(self.N):
            ...
            k1 = self.f(xi, ui)
            k2 = self.f(xi + 0.5*self.T*k1, ui)
            k3 = self.f(xi + 0.5*self.T*k2, ui)
            k4 = self.f(xi + self.T*k3, ui)
