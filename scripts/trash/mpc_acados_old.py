...
class MobileRobotOptimizer(object):
    def __init__(self, gazebo = False):
        ...
        config_path = os.path.join(cur_path, 'config/mpc_config.yaml')
        self.solver, self.integrator, self.T, self.N, self.t_horizon = self.create_solver(config_path)
        
        # cur_path = os.path.dirname(os.path.realpath(__file__))
        # parking_config_path = os.path.join(cur_path, 'config/mpc_config_park.yaml')
        # self.parking_solver, _, self.T_parking, self.N_parking, self.t_horizon_parking = self.create_solver(parking_config_path)
    ...
    def create_solver(self, config_path):
...
if __name__ == '__main__':
    ...
    # cur_path = os.path.dirname(os.path.realpath(__file__))
    # parking_config_path = os.path.join(cur_path, 'config/mpc_config_park.yaml')
    # mpc.parking_solver, _, mpc.T, mpc.N, mpc.t_horizon = mpc.create_solver(parking_config_path)
    # mpc.current_state = np.array([1, 0, np.pi])
    ...
    print("done")
    mpc.draw_result(mpc.compute_stats(), 0, 15, 0, 15)
