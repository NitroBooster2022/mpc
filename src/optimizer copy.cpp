...
int MobileRobotController::run() {
    ...
    int hsy = 0;
    while(1) {
        if(target_waypoint_index >= num_waypoints || hsy > 500) break;
        ...
        // x_current[0] = x_current[0] + u_current[0] * cos(x_current[2]) * 0.125;
        // x_current[1] = x_current[1] + u_current[0] * sin(x_current[2]) * 0.125;
        // x_current[2] = x_current[2] + u_current[0]/0.27*tan(u_current[1]) * 0.125;
        for(int i=0; i<nu; i++) {
...
int MobileRobotController::find_next_waypoint() {
    ...
    current_state.head(2) << x_current[0], x_current[1];
